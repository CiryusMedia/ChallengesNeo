...
public class TimerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if ((sender instanceof Player player)) {
            ...
            if (args.length == 0) {
                plugin.log("Opening timer gui for player " + player.getName(), DebugLevelOld.LEVEL_3);
            ...
            plugin.log("Sender isn't a player and didn't provide enough args", DebugLevelOld.LEVEL_3);
            return true;
        }
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "pause":
            case "stop":
                plugin.log("Handling timer paused/stop command", DebugLevelOld.LEVEL_3);
                if (!timer.isRunning()) {
                    plugin.log("Timer already paused", DebugLevelOld.LEVEL_3);
                    sender.sendMessage(Texts.PREFIX + ChatColor.RED + "Timer is already paused!");
                    break;
                }

                plugin.log("Timer paused", DebugLevelOld.LEVEL_3);
                timer.setRunning(false);
                sender.sendMessage(Texts.PREFIX + ChatColor.YELLOW + "Timer is now " + ChatColor.RED + "paused!");
                break;

            case "set":
                plugin.log("Handling timer set command", DebugLevelOld.LEVEL_3);
                if (args.length != 2) {
                    plugin.log("Not enough arguments for timer set command", DebugLevelOld.LEVEL_3);
                    sender.sendMessage(Texts.PREFIX + ChatColor.RED + "Wrong number of arguments!");
                    break;
                }

                plugin.log("Setting timer", DebugLevelOld.LEVEL_3);
                try {
                    timer.setTime(Integer.parseInt(args[1]));
                    plugin.log("Timer successfully set", DebugLevelOld.LEVEL_3);
                    sender.sendMessage(Texts.PREFIX + ChatColor.YELLOW + "Timer has been set to " + ChatColor.AQUA + timer.getTime());
                } catch (NumberFormatException e) {
                    plugin.log("Time is not a number", DebugLevelOld.LEVEL_3);
            ...
            case "load": {
                plugin.log("Loading timer time", DebugLevelOld.LEVEL_3);
            ...
            case "reset": {
                plugin.log("Resetting timer", DebugLevelOld.LEVEL_3);
                timer.setRunning(false);
                timer.setTime(0);
            ...
            case "display": {
                plugin.log("Handling timer display command", DebugLevelOld.LEVEL_3);
                if (args.length >= 2) {
                    displayHandler(sender, args);
                } else {
                    plugin.log("Not enough arguments for timer display command", DebugLevelOld.LEVEL_3);
            ...
            case "color":
                plugin.log("Handling timer color command", DebugLevelOld.LEVEL_3);
                if (args.length == 1 && sender instanceof Player player) {
                    plugin.log("Opening inventory for player", DebugLevelOld.LEVEL_3);
    ...
    private void displayHandler(CommandSender sender, String[] args) {
        plugin.log("Enough args to handle", DebugLevelOld.LEVEL_3);
        switch (args[1].toLowerCase(Locale.ROOT)) {
            case "running":
                plugin.log("Handling running timer visibility", DebugLevelOld.LEVEL_3);
                ...
                break;
            case "paused":
                plugin.log("Handling paused timer visibility", DebugLevelOld.LEVEL_3);
    ...
    private void colorHandler(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            if (colorType == null){
                plugin.log("Invalid args", DebugLevelOld.LEVEL_3);
                sender.sendMessage(Texts.PREFIX + Texts.INVALID_ARGUMENTS);
            } else if (args.length == 2 && sender instanceof Player player) {
                if (colorType.equalsIgnoreCase("running")) {
                    plugin.log("Opening timer running color gui for player", DebugLevelOld.LEVEL_3);
                    player.openInventory(Inventories.timerRunningColorGUI);
                } else {
                    plugin.log("Opening timer paused color gui for player", DebugLevelOld.LEVEL_3);
